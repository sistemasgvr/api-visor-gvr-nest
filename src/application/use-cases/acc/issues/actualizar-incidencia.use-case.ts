...
export class ActualizarIncidenciaUseCase {
  ...
  async execute(
    ...
    try {
      const issueAnterior =
        await this.autodeskApiService.obtenerIncidenciaPorId(
          accessToken,
          projectId,
          issueId,
        );
    ...
    if (dto.description !== undefined)
      updateData.description = dto.description.substring(0, 1000);
    ...
    if (dto.locationDetails !== undefined)
      updateData.locationDetails = dto.locationDetails.substring(0, 250);
    if (dto.published !== undefined) updateData.published = dto.published;
    if (dto.deleted !== undefined) updateData.deleted = dto.deleted;

    const resultado = await this.autodeskApiService.actualizarIncidencia(
      accessToken,
      projectId,
      issueId,
      updateData,
          ...
          datosAnteriores,
          {
            ...
            title:
              updateData.title !== undefined
                ? updateData.title
                : datosAnteriores?.title,
            status:
              updateData.status !== undefined
                ? updateData.status
                : datosAnteriores?.status,
            published:
              updateData.published !== undefined
                ? updateData.published
                : datosAnteriores?.published,
          },
          ipAddress,
          userAgent,
          {
