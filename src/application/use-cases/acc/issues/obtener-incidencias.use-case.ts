...
export class ObtenerIncidenciasUseCase {
  ...
  async execute(
    ...
    if (dto.filter_assignedTo)
      filters['filter[assignedTo]'] = dto.filter_assignedTo;
    if (dto.filter_issueTypeId)
      filters['filter[issueTypeId]'] = dto.filter_issueTypeId;
    if (dto.filter_rootCauseId)
      filters['filter[rootCauseId]'] = dto.filter_rootCauseId;
    if (dto.filter_locationId)
      filters['filter[locationId]'] = dto.filter_locationId;
    ...
    try {
      resultado = await this.autodeskApiService.obtenerIncidencias(
        accessToken,
        projectId,
        filters,
      );
      ...
      if (filterUrn && error.message?.includes('500')) {
        ...
        resultado = await this.autodeskApiService.obtenerIncidencias(
          accessToken,
          projectId,
          filtersWithoutLinkedDoc,
        );

        // Filtrar manualmente por documento vinculado
        const incidencias =
          resultado?.data?.results || resultado?.results || [];
      ...
      const incidenciasEnriquecidas = await Promise.all(
        incidencias.map(async (issue: any) => {
          try {
            ...
            if (
              usuariosAsignados &&
              usuariosAsignados.success &&
              usuariosAsignados.data &&
              usuariosAsignados.data.length > 0
            ) {
