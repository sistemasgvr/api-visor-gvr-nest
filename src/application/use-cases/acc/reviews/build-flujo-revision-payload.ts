import {
  CrearWorkflowDto,
  WorkflowCandidatoInput,
} from '../../../dtos/acc/reviews/crear-workflow.dto';

/** Resuelve nombre de opción BD acc_flujo_estado */
export function resolveEstadoFlujoNombre(
...
export function buildFlujoRevisionPayload(
  ...
  const pasos = steps.map((step: any, index: number) => {
    ...
    const minimoParticipantesRequeridos =
      groupReview?.min ??
      (revisionGrupalHabilitada && asignacionesForPaso.length > 0
    ...
    return {
      ...
      exigenciaAprobacion: revisionGrupalHabilitada
        ? 'Mínimo K de N'
        : 'Uno basta',
  ...
  const carpetaDestinoCopiaReferencia = String(
    copy.folderUrn ??
      (copy as { targetFolderId?: string }).targetFolderId ??
      '',
  ).trim();
  ...
  const allowApproversChange =
    (copy as { allowApproversChangeMarkups?: boolean })
      .allowApproversChangeMarkups !== false;
  const disableOverrideMarkup = (
    copy as { disableOverrideMarkupSetting?: boolean }
  ...
  const actualizarAtributosAlCerrarRevision =
    !!updateAttrs.enableAttachedAttributes;
  ...
  return {
    ...
    permitirEdicionIniciador:
      dto.additionalOptions?.allowInitiatorToEdit ?? false,
    ...
    ...(copyEnabled
      ? {
          ...
          carpetaDestinoCopiaReferencia:
            carpetaDestinoCopiaReferencia.length > 0
              ? carpetaDestinoCopiaReferencia
              : null,
