...
export class CrearWorkflowUseCase {
  ...
  async execute(
    ...
    try {
      const perfil =
        await this.autodeskApiService.obtenerPerfilUsuarioAcc(accessToken);
    ...
    if (Object.keys(copyFilesOptions).length === 0)
      copyFilesOptions.enabled = false;
    ...
    if ((dto.additionalApprovalStatusOptions as any[])?.length) {
      bodyAutodesk.additionalApprovalStatusOptions =
        dto.additionalApprovalStatusOptions;
    }

    console.log(
      '[CrearWorkflow] Body → Autodesk:',
      JSON.stringify(bodyAutodesk, null, 2),
    );

    const resultado = await this.autodeskApiService.crearWorkflow(
      accessToken,
      projectId,
      bodyAutodesk,
    );
