...
export class ExportarRevisionesPdfUseCase {
  constructor(private readonly exportacionRevisionesPdfService: ExportacionRevisionesPdfService) {}
  ...
  async execute(
    ...
    const data = await this.exportacionRevisionesPdfService.generarPdf(userId, projectId, query);
