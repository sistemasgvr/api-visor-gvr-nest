...
export class ObtenerRevisionPorIdUseCase {
  ...
  async execute(
    ...
    if (row) {
      ...
      if (fileIds.length > 0) {
        try {
          const versionRows = await this.dbFunctionService.executeQuery<{
            id: number;
            urnversionacc: string;
          }>(
          ...
          const vmap = new Map(
            versionRows.map((r) => [
              Number(r.id),
              String(r.urnversionacc ?? '').trim(),
            ]),
          );
          rawFiles = rawFiles.map((f) => {
            const vu =
              String(f.versionUrn ?? '').trim() || vmap.get(Number(f.id)) || '';
      ...
      const needsItemFromVersion = (f: any) =>
        !String(f.urnItemAcc ?? '').trim() &&
        String(f.versionUrn ?? '').trim().length > 0;
      const needsNameFromUrn = (f: any) =>
        String(f.name ?? '').startsWith('urn:');
      const needsAnyAccEnrich = rawFiles.some(
        (f) => needsItemFromVersion(f) || needsNameFromUrn(f),
      );
      ...
      if (needsAnyAccEnrich) {
        try {
          const accessToken =
            await this.obtenerTokenValidoHelper.execute(userId);
          ...
          enrichedFiles = await Promise.all(
            rawFiles.map(async (f) => {
              ...
              if (needsItemFromVersion(f) && versionUrn) {
                try {
                  const versionResp =
                    await this.autodeskApiService.obtenerVersionPorId(
                      accessToken,
                      projectId,
                      versionUrn,
                    );
                  const itemId =
                    versionResp?.data?.relationships?.item?.data?.id;
                  if (itemId) {
                    const itemStr = String(itemId);
                    next = { ...next, urnItemAcc: itemStr };
                    this.dbFunctionService
                      .executeQuery(
              ...
              if (rawName.startsWith('urn:')) {
                try {
                  ...
                  const displayName: string =
                    attrs.displayName ?? attrs.name ?? rawName;
                  const versionNum: number | null = attrs.versionNumber ?? null;
                  const versionLabel = versionNum
                    ? `V${versionNum}`
                    : (next.version ?? 'V1');
      ...
      return {
        ...
        currentStep: {
          ...
          candidates: Array.isArray(row.candidates_current_json)
            ? row.candidates_current_json
            : [],
        },
    ...
    try {
      return await this.autodeskApiService.obtenerRevisionPorId(
        accessToken,
        projectId,
        reviewId,
      );
