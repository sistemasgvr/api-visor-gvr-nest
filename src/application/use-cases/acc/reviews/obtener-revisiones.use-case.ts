...
export class ObtenerRevisionesUseCase {
  ...
  async execute(
    ...
    const distinctFlowIds = Array.from(
      new Set(
        rows.map((r) => String(r.workflow_id)).filter((x) => /^\d+$/.test(x)),
      ),
    );
    ...
    let mapped = rows.map((r) => {
      ...
      const gvrNextActionBy =
        candidatesByFlowStep.get(`${workflowId}:${currentStep}`)?.map((c) => ({
      ...
      return {
        ...
        currentStepId:
          r.current_step_number != null ? String(r.current_step_number) : null,
        ...
        nextActionBy: {
          ...
          candidates: {
            roles: [],
            users: gvrNextActionBy.map((c) => ({
              autodeskId: String(c.idUsuario),
              name: c.nombre,
            })),
            companies: [],
          },
    ...
    if (dto.filter_sequenceId !== undefined)
      mapped = mapped.filter((x) => x.sequenceId === dto.filter_sequenceId);
    if (dto.filter_archived !== undefined)
      mapped = mapped.filter((x) => x.archived === dto.filter_archived);
