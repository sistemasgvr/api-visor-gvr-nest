...
export class CronAlertaActividadesSinValidarUseCase {
    private readonly logger = new Logger(CronAlertaActividadesSinValidarUseCase.name);
    ...
    async execute(fecha: string): Promise<CronAlertaActividadesSinValidarResult> {
        ...
        const result = await this.controlOperativoRepository.ejecutarCronAlertaActividadesSinValidar(f);
        ...
        if (result.totalActividades === 0 || result.usuariosANotificar.length === 0) {
            return result;
        }
    ...
    private buildNotification(grupo: GrupoCoordinadorSinValidar): Record<string, unknown> {
