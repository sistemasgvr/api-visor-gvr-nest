...
export class ObtenerActividadUseCase {
    ...
    async execute(input: ObtenerActividadInput | number): Promise<ActividadDetalle | null> {
        const idActividad = typeof input === 'number' ? input : input.idActividad;
        const contextoValidacion = typeof input === 'object' && input.contextoValidacion === true;
        ...
        if (contextoValidacion) {
            ...
            const idRevisor = await this.controlOperativoRepository.obtenerIdTrabajadorPorIdUsuario(idUsuario);
            if (idRevisor == null) {
                return null;
            }
