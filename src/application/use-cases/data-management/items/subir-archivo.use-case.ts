  ...
  async execute(
    ...
    if (!folderId) {
      throw new BadRequestException(
        'El ID de la carpeta (folderId) es requerido',
      );
    }
    ...
    if (!storageIdMatch || storageIdMatch.length !== 3) {
      throw new BadRequestException(
        `Formato de storage ID inválido: ${storageId}`,
      );
    }
    ...
    if (existingItem) {
      ...
      itemResult = {
        data: {
          ...existingItem,
          attributes: { ...existingItem.attributes, displayName: fileName },
        },
        included: [],
      };
    } else {
        ...
        try {
          ...
          try {
            ...
            if (perfil) {
              ...
              if (!rolNombre) {
                const roles =
                  perfil.roles && Array.isArray(perfil.roles)
                    ? perfil.roles
                    : [];
                rolNombre =
          ...
          console.error(
            'Error registrando auditoría FILE_VERSION_SAVE (subida):',
            error,
          );
        }
      }
