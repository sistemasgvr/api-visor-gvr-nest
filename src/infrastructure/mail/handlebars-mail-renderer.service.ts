...
import type { IMailRenderer, RenderedMail } from '../../domain/services/mail-renderer.interface';
...
export class HandlebarsMailRendererService implements IMailRenderer {
    ...
    private readonly partialCache = new Map<string, handlebars.TemplateDelegate>();
    ...
    async render(
        ...
        try {
            ...
            const subjectTpl = handlebars.compile(subjectOverride ?? def.defaultSubject);
            const subject = subjectTpl(variables).trim();
            if (!subject) {
                this.logger.warn(`Asunto vacío para template=${templateId}, usando id como fallback`);
            }
