...
import type { IMailTransport, MailTransportMessage } from '../../domain/services/mail-transport.interface';
...
export class NodemailerMailTransportService implements IMailTransport {
    ...
    private getTransporter(): Transporter {
        ...
        const host = (this.configService.get<string>('MAIL_SMTP_HOST') ?? '').trim();
        const port = this.configService.get<number>('MAIL_SMTP_PORT') ?? 587;
        const secure =
            this.configService.get<string>('MAIL_SMTP_SECURE') === 'true' ||
            port === 465;
        const user = (this.configService.get<string>('MAIL_SMTP_USER') ?? '').trim();
    ...
    async send(message: MailTransportMessage): Promise<void> {
        const fromAddress =
            this.configService.get<string>('MAIL_FROM_ADDRESS') ?? 'no-reply@localhost';
        const fromName = this.configService.get<string>('MAIL_FROM_NAME') ?? 'Visor GVR';
        ...
        try {
            ...
            } else if (code === 'ECONNECTION' || code === 'ETIMEDOUT' || /ECONNREFUSED/i.test(raw)) {
                message =
