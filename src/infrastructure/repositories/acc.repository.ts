...
export class AccRepository implements IAccRepository {
  ...
  async guardarToken3Legged(
    ...
    const result = await this.databaseFunctionService.callFunctionSingle<any>(
      ...
      [idUsuario, tokenAcceso, tokenRefresco, expiraEn, tipoToken],
    );
  ...
  async obtenerToken3LeggedPorUsuario(
    idUsuario: number,
  ): Promise<AccToken | null> {
  ...
  async listarTokensActivosParaRefresh(): Promise<
    { id: number; idUsuario: number; tokenRefresco: string }[]
  > {
    ...
    return (rows || [])
      .map((r: any) => {
        const tokenRefresco = (r.v_token_refresco ?? r.token_refresco ?? '')
          .toString()
          .trim();
        return {
          id: r.v_id ?? r.id,
