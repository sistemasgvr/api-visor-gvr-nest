...
export class PdfIssueDetailTemplate {
  ...
  static async draw(doc: PDFDoc, data: IssueDetailData): Promise<void> {
    ...
    // Campos estándar
    const campos = this.obtenerCamposEstandar(
      data.incidencia,
      data.usuarioCreador,
      data.emailCreador,
    );
    ...
    if (
      data.incluirCamposPersonalizados &&
      data.incidencia.customAttributes?.length > 0
    ) {
      y = this.dibujarCamposPersonalizados(
        doc,
        y,
        data.incidencia.customAttributes,
      );
    }

    // Vínculos de archivo (si está habilitado)
    if (
      data.incluirVinculosArchivo &&
      data.incidencia.linkedDocuments?.length > 0
    ) {
    ...
    if (data.incluirComentarios && data.incidencia.comentarios?.length > 0) {
      y = this.dibujarComentarios(
        doc,
        y,
        data.incidencia.comentarios,
        data.usuarioCreador,
      );
    }
  }
  ...
  private static async dibujarAdjuntos(
    doc: PDFDoc,
    startY: number,
    ...
    // Título de sección
    y = PdfSectionComponent.drawTitle(
      doc,
      y,
    ...
    const anchoTotalFila =
      columnasPorFila * anchoFoto + (columnasPorFila - 1) * espacioEntreFotos;
    ...
    const alturaMetadatos =
      PdfFontsConfig.FONT_SIZE_MD + 12 + 12 + 12 + PdfLayoutConfig.SPACING_SM;
    ...
    for (let i = 0; i < fotos.length; i++) {
      ...
      if (esPrimeraColumna && i > 0) {
        y = PdfLayoutHelper.ensureSpace(
          doc,
          alturaTotalPorImagen + PdfLayoutConfig.SPACING_MD,
          y,
        );
        y += PdfLayoutConfig.SPACING_MD;
      ...
      const x =
        ...
        offsetX +
        columna * (anchoFoto + espacioEntreFotos);
      ...
      // Creador
      const creador =
        fotos[i].createdByReal || fotos[i].createdBy || data.usuarioCreador;
      PdfFontsConfig.applyTextStyle(doc, {
        size: PdfFontsConfig.FONT_SIZE_SM,
        color: PdfFontsConfig.COLOR_GRAY,
      });
  ...
  private static dibujarComentarios(
    ...
    comentarios.forEach((comentario: any) => {
      ...
      const textoComentario =
        PdfTextHelper.procesarTextoComentario(textoCompleto);
  ...
  private static dibujarAvatarConIniciales(
    ...
    doc.circle(centerX, centerY, radius).fillColor(backgroundColor).fill();
  ...
  private static obtenerCamposEstandar(
    ...
    // Tipo
    let tipoTexto =
      incidencia.issueSubtypeName ||
      incidencia.issueTypeName ||
      'Design > Design';
    ...
    // Asignado a
    const asignadoA =
      incidencia.assignedToRealMultiple
        ?.map((u: any) => u.usuario)
        .join(', ') ||
    ...
    campos.push({
      label: 'Detalles de la ubicación',
      value: incidencia.locationDetails || '—',
    });
    ...
    if (incidencia.dueDate) {
      ...
      const diasTarde = Math.floor(
        (new Date().getTime() - fechaVencimiento.getTime()) /
          (1000 * 60 * 60 * 24),
      );
      const fechaTexto = this.formatearFecha(fechaVencimiento, false);
      const vencimientoTexto =
        diasTarde > 0 ? `${fechaTexto} (${diasTarde} días tarde)` : fechaTexto;
  ...
  private static dibujarInformacionGeneralPlano(
    doc: PDFDoc,
    startY: number,
    incidencia: any,
  ): number {
    ...
    if (incidencia.linkedDocuments?.[0]) {
      ...
      if (linkedDoc.details?.viewable?.name) {
        campos.push({
          ...
          value: linkedDoc.details.viewable.name,
        });
      }
      if (linkedDoc.urn) {
    ...
    if (campos.length > 0) {
      ...
      doc.text(
        'No hay información disponible del plano',
        PdfLayoutConfig.MARGIN_LEFT,
        y,
        {
          width: PdfLayoutConfig.USABLE_WIDTH,
  ...
  private static dibujarVinculosArchivo(
    doc: PDFDoc,
    startY: number,
    linkedDocuments: any[],
  ): number {
    ...
    linkedDocuments.forEach((linkedDoc: any, index: number) => {
      ...
      if (linkedDoc.createdBy)
        campos.push({ label: 'Creado por', value: linkedDoc.createdBy });
      if (linkedDoc.createdAtVersion)
        campos.push({
          label: 'Versión',
          value: String(linkedDoc.createdAtVersion),
        });
  ...
  private static dibujarOtrasReferencias(
    doc: PDFDoc,
    startY: number,
    incidencia: any,
  ): number {
    ...
    if (incidencia.issueSubtypeId) {
      campos.push({
        ...
        value: incidencia.issueSubtypeId,
      });
    }
    ...
    if (incidencia.gpsCoordinates) {
      campos.push({
        ...
        value: incidencia.gpsCoordinates,
      });
    }
    ...
    if (campos.length > 0) {
      ...
      doc.text(
        'No hay otras referencias disponibles',
        PdfLayoutConfig.MARGIN_LEFT,
        y,
        {
          width: PdfLayoutConfig.USABLE_WIDTH,
