...
export class AccReviewsController {
  ...
  async obtenerRevisiones(
    ...
    const resultado = await this.obtenerRevisionesUseCase.execute(
      userId,
      projectId,
      dto,
    );
  ...
  async exportarRevisionesPdf(
    ...
    const resultado = await this.exportarRevisionesPdfUseCase.execute(
      userIdNumero,
      projectId,
      query,
    );
  ...
  async exportarRevisionDetallePdf(
    ...
    if (!projectId)
      throw new BadRequestException('El ID del proyecto es requerido');
    if (!reviewId)
      throw new BadRequestException('El ID de la revisi?n es requerido');
    ...
    const resultado = await this.exportarRevisionDetallePdfUseCase.execute(
      userIdNumero,
      projectId,
      reviewId,
    );
  ...
  async obtenerRevisionPorId(
    ...
    const resultado = await this.obtenerRevisionPorIdUseCase.execute(
      userId,
      projectId,
      reviewId,
    );
  ...
  async obtenerWorkflowRevision(
    ...
    const resultado = await this.obtenerWorkflowRevisionUseCase.execute(
      userId,
      projectId,
      reviewId,
    );
    ...
    return ApiResponseDto.success(
      resultado,
      'Workflow de revisi?n obtenido exitosamente',
    );
  }
  ...
  async obtenerProgresoRevision(
    ...
    const resultado = await this.obtenerProgresoRevisionUseCase.execute(
      userId,
      projectId,
      reviewId,
    );
    ...
    return ApiResponseDto.success(
      resultado,
      'Progreso de revisi?n obtenido exitosamente',
    );
  }
  ...
  async obtenerVersionesRevision(
    ...
    const resultado = await this.obtenerVersionesRevisionUseCase.execute(
      userId,
      projectId,
      reviewId,
    );
    ...
    return ApiResponseDto.success(
      resultado,
      'Versiones de revisi?n obtenidas exitosamente',
    );
  }
  ...
  async obtenerReferenciasRevision(
    ...
    if (isNaN(idRevision))
      throw new BadRequestException('ID de revisi?n inv?lido');

    const resultado =
      await this.obtenerReferenciasRevisionUseCase.execute(idRevision);
    return ApiResponseDto.success(
      resultado,
  ...
  async agregarReferenciasRevision(
    ...
    const resultados = await Promise.all(
      items.map((item) =>
        this.agregarReferenciaRevisionUseCase.execute(
          idRevision,
          projectId,
          item,
          userId,
    ...
    return ApiResponseDto.created(
      resultados,
      'Referencia(s) agregada(s) exitosamente',
    );
  }
  ...
  async eliminarReferenciaRevision(
    ...
    const resultado = await this.eliminarReferenciaRevisionUseCase.execute(
      refId,
      userId,
    );
  ...
  async anularRevisionEntire(
    ...
    const resultado = await this.anularRevisionEntireUseCase.execute(
      userId,
      projectId,
      idRevision,
      dto,
  ...
  async saltarPasoRevision(
    ...
    const resultado = await this.saltarPasoRevisionUseCase.execute(
      userId,
      projectId,
      idRevision,
      dto,
  ...
  async volverPasoAnterior(
    ...
    const resultado = await this.volverPasoAnteriorRevisionUseCase.execute(
      userId,
      projectId,
      idRevision,
      dto,
  ...
  async iniciarPasoRevision(
    ...
    const userId = user?.sub || user?.id;
    if (!userId) throw new BadRequestException('User ID es requerido');
    const resultado = await this.iniciarPasoRevisionUseCase.execute(
      userId,
      projectId,
      idRevision,
    );
  ...
  async delegarPasoRevision(
    ...
    const userId = user?.sub || user?.id;
    if (!userId) throw new BadRequestException('User ID es requerido');
    const resultado = await this.delegarPasoRevisionUseCase.execute(
      userId,
      projectId,
      idRevision,
    );
  ...
  async enviarResenaPaso(
    ...
    const userId = user?.sub || user?.id;
    if (!userId) throw new BadRequestException('User ID es requerido');
    const resultado = await this.enviarResenaPasoUseCase.execute(
      userId,
      projectId,
      idRevision,
      dto,
  ...
  async notificarRevisoresRevision(
    ...
    return ApiResponseDto.success(
      resultado,
      'Notificaciones encoladas correctamente',
    );
  }
  ...
  async getComentariosArchivo(
    ...
    if (isNaN(idRevision))
      throw new BadRequestException('ID de revisi?n inv?lido');
    if (isNaN(idArchivo))
      throw new BadRequestException('ID de archivo inv?lido');
    const data = await this.getComentariosArchivoUseCase.execute(
      idRevision,
      idArchivo,
    );
  ...
  async addComentarioArchivo(
    ...
    const userId = user?.sub || user?.id;
    if (!userId) throw new BadRequestException('User ID es requerido');
    const resultado = await this.addComentarioArchivoUseCase.execute(
      userId,
      idRevision,
      idArchivo,
      dto,
