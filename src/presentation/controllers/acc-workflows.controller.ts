...
import {
  ...
  type GuardarWorkflowCandidatosDto,
} from '../../application/use-cases/acc/reviews/guardar-workflow-candidatos.use-case';
...
export class AccWorkflowsController {
  ...
  async obtenerWorkflows(
    ...
    const resultado = await this.obtenerWorkflowsUseCase.execute(
      userId,
      projectId,
      dto,
    );
  ...
  async obtenerCandidatos(
    ...
    if (!projectId)
      throw new BadRequestException('El ID del proyecto es requerido');
    if (!workflowId)
      throw new BadRequestException('El ID del workflow es requerido');
  ...
  async cambiarEstadoWorkflow(
    ...
    const resultado = await this.cambiarEstadoFlujoGvrUseCase.execute(
      projectId,
      workflowId,
      dto,
    );
  ...
  async obtenerWorkflowPorId(
    ...
    const resultado = await this.obtenerWorkflowPorIdUseCase.execute(
      userId,
      projectId,
      workflowId,
    );
  ...
  async crearWorkflow(
    ...
    const resultado = await this.crearFlujoRevisionGvrUseCase.execute(
      userId,
      projectId,
      dto,
    );
  ...
  async guardarCandidatos(
    ...
    return ApiResponseDto.success(
      resultado,
      'Candidatos guardados exitosamente',
    );
  }
}
