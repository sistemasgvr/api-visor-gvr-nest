...
export class DashboardController {
    ...
    async obtenerCantidadActividadesPendientes() {
        const data = await this.obtenerCantidadActividadesPendientesUseCase.execute();
    ...
    async obtenerCantidadActividadesObservadas() {
        const data = await this.obtenerCantidadActividadesObservadasUseCase.execute();
    ...
    async obtenerCantidadActividadesRechazadas() {
        const data = await this.obtenerCantidadActividadesRechazadasUseCase.execute();
    ...
    async obtenerCantidadTrabajadoresPorProyecto() {
        const data = await this.obtenerCantidadTrabajadoresPorProyectoUseCase.execute();
        return ApiResponseDto.success(data, 'Cantidad de trabajadores por proyecto');
    }

    @Get('cantidad-jornadas-completas-semana')
    @HttpCode(HttpStatus.OK)
    async obtenerCantidadJornadasCompletasSemana() {
        const data = await this.obtenerCantidadJornadasCompletasSemanaUseCase.execute();
        return ApiResponseDto.success(data, 'Cantidad de jornadas completas de la semana');
    }
    ...
    async obtenerCantidadTrabajadoresConectadoSemana() {
        const data = await this.obtenerCantidadTrabajadoresConectadoSemanaUseCase.execute();
        return ApiResponseDto.success(data, 'Cantidad de trabajadores conectados por semana');
    }
