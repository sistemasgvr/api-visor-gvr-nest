import { Controller, Get, Post, Delete, UseGuards, Request, Query, Param, ParseIntPipe, Logger } from '@nestjs/common';
...
export class NotificacionesController {
    ...
    async listar(@Request() req: any, @Query('tipo') tipo?: string) {
        ...
        const notificaciones = await this.obtenerNotificacionesUseCase.execute(userId, tipo);
        this.logger.log(`[NOTIF] GET / → userId=${userId} devuelve ${notificaciones?.length ?? 0}`);
        return {
            status: 200,
