...
export class OfficeDocumentController {
  ...
  async generateToken(
    ...
    const accToken = await this.accRepository.obtenerToken3LeggedPorUsuario(
      user.sub,
    );
    if (!accToken) {
   ...
   */
  @Options('view/:token')
  async viewDocumentOptions(@Res() res: Response) {
   ...
   */
  @Head('view/:token')
  async viewDocumentHead(@Param('token') token: string, @Res() res: Response) {
    this.logger.log(
      `HEAD request recibido para token: ${token.substring(0, 8)}...`,
    );
  ...
  async viewDocument(@Param('token') token: string, @Res() res: Response) {
    this.logger.log(
      `GET request recibido para token: ${token.substring(0, 8)}...`,
    );
    ...
    if (!tokenData) {
      this.logger.warn(
        `Token inválido o expirado: ${token.substring(0, 8)}...`,
      );
      throw new NotFoundException('Token inválido o expirado');
    }

    this.logger.log(
      `Token válido para archivo: ${tokenData.fileName}, userId: ${tokenData.userId}`,
    );
    ...
    try {
      ...
      const bufferLength = Buffer.isBuffer(fileBuffer)
        ? fileBuffer.length
        : (fileBuffer as ArrayBuffer).byteLength;
      this.logger.log(
        `Archivo descargado exitosamente, tamaño: ${bufferLength} bytes`,
      );
      ...
      const toSend = Buffer.isBuffer(fileBuffer)
        ? fileBuffer
        : Buffer.from(fileBuffer as ArrayBuffer);
      res.status(HttpStatus.OK).send(toSend);
    } catch (error: any) {
      this.logger.error(
        `Error sirviendo documento: ${error?.message}`,
        error?.stack,
      );
      if (error?.status) {
        throw error;
      ...
      throw new BadRequestException(
        error?.message || 'Error al obtener el documento',
      );
    }
  }
  ...
  private getBaseUrl(request: Request): string {
    ...
    const forwardedHost =
      (request.headers['x-forwarded-host'] as string) || request.get('host');
