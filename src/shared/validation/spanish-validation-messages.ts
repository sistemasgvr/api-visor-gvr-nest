...
const SENTENCE_PATTERNS: { test: RegExp; es: string }[] = [
    { test: /must be (?:a |an )?e-?mail/i, es: 'debe ser un correo electrónico válido' },
    { test: /must be a string/i, es: 'debe ser texto' },
    { test: /each value in .+ must be a string/i, es: 'cada valor debe ser texto' },
    { test: /each value in .+ must be a number/i, es: 'cada valor debe ser un número' },
    ...
    { test: /must be a date string/i, es: 'debe ser una fecha en formato ISO válida' },
    ...
    { test: /must be shorter than or equal to (\d+) characters/i, es: 'no puede superar $1 caracteres' },
    { test: /must be longer than or equal to (\d+) characters/i, es: 'debe tener al menos $1 caracteres' },
    { test: /must (?:be |contain )?longer? than or equal to (\d+) and (?:shorter|shorter) than or equal to (\d+) characters/i, es: 'debe tener entre $1 y $2 caracteres' },
    { test: /must be equal to (\d+) characters/i, es: 'debe tener exactamente $1 caracteres' },
    { test: /should not be empty/i, es: 'es obligatorio' },
    { test: /should not (?:be |be null|be undefined|null|empty)/i, es: 'es obligatorio' },
    { test: /(property )?.+ (should not exist|should not be provided)/i, es: 'incluye datos no permitidos' },
    { test: /must (?:be |a )?positive (?:a )?number/i, es: 'debe ser un número mayor que cero' },
    { test: /must (?:be |a )?non-?negative (?:a )?number/i, es: 'debe ser un número mayor o igual a cero' },
    { test: /must not (?:be |)greater than (\d+)/i, es: 'no puede ser mayor que $1' },
    { test: /must not (?:be |)less than (\d+)/i, es: 'no puede ser menor que $1' },
    { test: /must be one of the following values: (.+)/i, es: 'debe ser uno de los valores permitidos' },
    { test: /must be a valid ISO 8601 date string/i, es: 'debe ser una fecha ISO 8601 válida' },
    { test: /is not a valid (?:military )?time/i, es: 'no es una hora válida' },
    { test: /must be a (?:url|URL)\b/i, es: 'debe ser una URL válida' },
    { test: /must be a (?:military|valid) time string/i, es: 'debe ser una hora válida' },
    { test: /must (?:be |)match\?? /i, es: 'no cumple el formato requerido' },
    { test: /must (?:be |)a?(?:n)? mimetype/i, es: 'el tipo de archivo no es válido' },
    { test: /must (?:be |)a?(?:n)?? enum/i, es: 'el valor no es una opción válida' },
];
    ...
    switch (constraintKey) {
        ...
        case 'max': {
            const t = defaultMessage
                .replace(
                    new RegExp(`^${lastKey(property).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+`, 'i'),
                    '',
                )
                .trim();
        ...
        case 'isMimeType': {
            const m = defaultMessage?.replace(
                new RegExp(`^${lastKey(property).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+`),
                '',
            );
